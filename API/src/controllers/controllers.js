...
const allInfo = async (email) => {
  const apiInfo = await getApiUsers(email); //trae la info de la API luego de cargarse (por eso el await)
  const dbInfo = await getDbInfo(email); //trae la info de la BBDD luego de cargarse (por eso el await)
...
module.exports = { allInfo, allInfoCourses };
